...
class AgentService:
    ...
    async def public_turn(
        ...
        *,
        ...
        reply, raw_config = await self._backend.run(message=message, config=merged)
        new_config = merge_model_config_into_base(base=merged, model_config=raw_config)
