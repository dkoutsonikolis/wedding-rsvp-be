...
- **Versioned API** (`/api/v1/…`):
  ...
  - `POST /api/v1/auth/register` — create user (`email`, `password` min 8 characters); returns `access_token`, `refresh_token`, `token_type` (`bearer`), `expires_in` / `refresh_expires_in` (seconds), and `user`
  ...
  - **Owner RSVP summary** (Bearer required) — `GET /api/v1/wedding-sites/{site_id}/rsvp-responses/summary` returns aggregate counts (`attending_count`, `declined_count`, `total_guests` = sum of `party_size` where attending, `total_responses`) plus up to **3** newest rows in `recent_activity` and up to **6** newest rows with non-empty `notes` in `guest_notes` (each preview row uses the same `RsvpResponseRead` shape as the list endpoint). **404** if the site id is missing or not yours.
  - **Owner RSVP list** (Bearer required) — `GET /api/v1/wedding-sites/{site_id}/rsvp-responses?limit=20&before_response_id=<uuid>` returns `{ items, next_before_response_id }` (newest first). Each item matches the public submit response shape (`id`, `name`, `email`, `phone`, `is_attending`, `party_size`, `notes`, `created_at`). Query `limit` defaults to **20**, max **50**; pass `before_response_id` from the previous page’s `next_before_response_id` to load older rows. Optional **`q`** (max **100** characters; trimmed; case-insensitive partial match on `name`, `email`, `phone`, `notes`) and **`is_attending`** (`true` / `false`; omit for all) filter the list before pagination. **404** if the site id is missing or not yours.
